...
task read_sheet {
  input {
    ...
    runtime {
      docker: "us-central1-docker.pkg.dev/velina-208320/jonah-slidetags/img@sha256:b8a590f6cb036db6862874c1750aade57370f8ab73a9af4f94aee423cb577d62"
      memory: "16 GB"
...
task RNAcounts {
  ...
  command <<<
    ...
    time stdbuf -oL -eL cellranger count \
      ...
      --create-bam=true                  \
      --nosecondary |& ts | tee -a ./RNAcounts.log
...
task spatial {
  ...
  command <<<
    ...
    then
      ...
      gcloud storage cp seurat.qs coords.csv summary.pdf df.qs data.list.qs gs://~{bucket}/05_SEURATS/~{bcl}/~{basename(RNApath)}/
      echo "true" > DONE
    else
