...
fn main() {
    ...
    for d in data { // d is (read, flag, rname_i, mapq, cb_i, ub_i)
        ...
        let umi_info: &mut UMIinfo = umi_dict.entry((d.rname_i, flag2strand(d.flag))).or_insert(UMIinfo::new()); // get the UMIinfo for the current (rname_i, strand)
        umi_info.reads += 1;
        ...
        while snv_idx < snv.len() && snv[snv_idx].read == d.read { // snv is (read, pos, alt, qual)
            ...
            let snv_i: SNVi = snv_table.get(&(d.rname_i, pos, alt));
            umi_info.snv_i.entry((snv_i, pos))
    ...
    // vec![&args[1]].write_vector(&meta_group, "input_file");
