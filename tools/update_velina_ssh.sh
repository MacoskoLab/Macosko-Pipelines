...
/Users/mshabet/google-cloud-sdk/bin/gcloud compute instances list  --project velina-208320 --filter='status=RUNNING' --format='csv[no-heading](name,networkInterfaces[0].accessConfigs[0].natIP)' > /tmp/gcloud_hosts_1
...
cat /tmp/gcloud_hosts | awk -F, '{print "Host "$1"@    HostName "$2"@    StrictHostKeyChecking no@    User mshabet@    IdentityFile ~/.ssh/google_compute_engine@"}' | tr '@' '\n' >> ~/.ssh/config
